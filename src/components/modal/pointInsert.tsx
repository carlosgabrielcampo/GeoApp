...
import { FormEvent, TouchEvent, useEffect, useMemo, WheelEvent } from "react";
...
export default function PointInsert({
  ...
  const blockBackgroundInteraction = (
  ...
  return (
    <div
      className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/40 p-4 touch-none"
      onTouchMove={blockBackgroundInteraction}
      onWheel={blockBackgroundInteraction}
    >
      <div className="flex h-[600px] w-[400px] flex-col rounded-2xl bg-white p-6 shadow-2xl touch-auto">
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 text-sm text-slate-700">
            <div className="flex items-center justify-between">
              ...
              >
                {isPickingCoordinates ? "Double click map..." : "Pick on map"}
              </button>
            </div>
            <div className="rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 font-mono text-xs text-slate-600">
              {selectedPoint?.geometry?.coordinates ? `[${selectedPoint?.geometry.coordinates[0]}, ${selectedPoint?.geometry.coordinates[1]}]` : "No coordinates selected"}
            </div>
            <div className="text-xs text-slate-500">
              {isPickingCoordinates
                ? "Double click anywhere on the map to place this point."
                : "Geometry type is fixed as Point."}
            </div>
          </div>
